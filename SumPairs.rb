def two_sum(nums, target)
    ...
    sorted.each_with_index do |el, idx_first|
        ...
        while end_range - start_range > 1 do

            trial_idx = start_range + ((end_range.to_f - start_range.to_f)/2).ceil
            puts trial_idx
            puts " #{sorted[trial_idx][0]} and #{find} #{start_range} #{end_range}"

            if sorted[trial_idx][0] == find
                result << sorted[trial_idx][0]
                result << el[1]
                break
